...
3. 반정규화 방법 및 기법
    ...
    2. 칼럼(속성) 반정규화(파생, 오류, 이력컬럼, PK →속성, 중복속성)
        - 중복칼럼 추가: 조인 감소를 위해
        - 파생칼럼 추가: 미리 값 계산해서 칼럼에 저장
        - 이력테이블칼럼 추가: 최근값 여부, 시작종료일자
        - PK에 의한 칼럼 추가: 단일 속성으로 구성된 복합 속성 PK를 분리
        - 응용시스템 오작동 위한 칼럼 추가: backup 컬럼 추가
    3. 관계 반정규화(중복관계추가)
